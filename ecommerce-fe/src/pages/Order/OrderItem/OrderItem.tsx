...
const OrderItem: FC<PropsType> = ({ perfume, quantity }): ReactElement => {
    return (
        <Col span={12}>
            <Card
                className={"menu-card"}
                cover={<img className={"menu-card-image"} alt={perfume.productTitle} src={perfume.filename} />}
            >
                <div className={"menu-content"}>
                    <Typography.Text strong>{perfume.productr}</Typography.Text>
                    <Typography.Text strong>{perfume.productTitle}</Typography.Text>
